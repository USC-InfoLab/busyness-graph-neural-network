...
class Model(nn.Module):
    ...
    def forward(self, x, static_features=None):
        ...
        edge_indices, edge_attrs = pyg_utils.dense_to_sparse(self.dist_adj)
        ...
        for stack_i in range(1, self.conv_layers_num):
            ...
            X_gnn = F.relu(X_gnn)
        gnn_embeddings = X_gnn.detach().clone()
        X_hat = torch.cat((X, X_gnn), dim=2)
        forecast = self.fc(X_hat)
        if forecast.size()[-1] == 1:
            return forecast.unsqueeze(1).squeeze(-1), attention.unsqueeze(0), (adj_mat_thresholded, adj_mat_unthresholded, embed_att, self.dist_adj, mhead_att_mat), gnn_embeddings
        else:
            return forecast.squeeze(1).permute(0, 2, 1).contiguous(), attention.unsqueeze(0), (adj_mat_thresholded, adj_mat_unthresholded, embed_att, self.dist_adj, mhead_att_mat), gnn_embeddings
